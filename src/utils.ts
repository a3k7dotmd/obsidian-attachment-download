...
// Full-content MD5 as uppercase hex, matching obsidian-attachment-management's ${md5}
// (its md5sum() uppercases). Used for the ${md5} filename variable and dedup comparison.
export function md5Full(contentData: ArrayBuffer): string {
  return crypto.createHash("md5").update(Buffer.from(contentData)).digest("hex").toUpperCase();
}
